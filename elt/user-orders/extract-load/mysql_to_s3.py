...
for table in ["USERS", "ORDERS"]:
    ...
    query = f"""select * from {table.lower()} where created_at > '{last_ingestion.strftime("%Y-%m-%d %H:%M:%S")}' or updated_at > '{last_ingestion.strftime("%Y-%m-%d %H:%M:%S")}'"""
    print(query)
    df_new = spark\
        .read\
        .format("jdbc")\
        .option("url", "jdbc:mysql://db:3306/usersorders?useSSL=false") \
